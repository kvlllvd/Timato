...
/// половину черточки, 55 — целую. Целые числа вместо дробей выбраны нарочно —
...
enum Segments {
    ...
    static func credit(minutes: Int) -> Int {
        ...
        // 55 минут — целая черточка, всё остальное рабочее (в том числе 25) — половина.
        return minutes >= 55 ? halvesPerSegment : 1
    }
...
enum Kind: Hashable {
    ...
    /// Пятиминутка — всегда отдых, всё остальное — работа.
    static func forMinutes(_ minutes: Int) -> Kind {
        minutes == Presets.rest ? .rest : .focus
    }
}
...
func nextAfterFinish(minutes: Int) -> Int? {
    Kind.forMinutes(minutes) == .focus ? Presets.rest : nil
}
